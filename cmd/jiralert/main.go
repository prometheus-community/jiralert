...
func main() {
	...
	http.HandleFunc("/alert", func(w http.ResponseWriter, req *http.Request) {
		...
		if retry, err := notify.NewReceiver(logger, conf, tmpl, client.Issue).Notify(&data, *hashJiraLabel, *updatePriority, *updateSummary, *updateDescription, *reopenTickets, *maxDescriptionLength); err != nil {
			var status int
			if retry {
